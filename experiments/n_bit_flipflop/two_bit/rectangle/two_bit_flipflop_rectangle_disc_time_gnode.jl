...
data, target = RNNTools.generate_data_n_bit_task_rectangle(task)
...
@save "two_bit_task_rectangle_data.bson" task data target
@save "two_stats_rectangle_"*string(idx)*".bson" losses_gnode val_losses_gnode grad_norms_gnode time_gnode lps_gnode
@save "two_params_rectangle_"*string(idx)*".bson" θ_opt_gnode θ_init_gnode